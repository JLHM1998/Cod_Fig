# -*- coding: utf-8 -*-
"""
Created on Wed Sep  4 15:35:42 2024
...
file_path = 'ETcSimulated.xlsx'  # Cambia esto a la ruta donde esté tu archivo
...
for treatment in ['CF', 'AWD5', 'AWD10', 'AWD20']:
    ...
    plt.scatter(group_data['ETc_Aquacrop'], group_data['Etc_Drone'], 
                color=treatment_colors[treatment], label=treatment)
    
    # Realizar regresión lineal para cada tratamiento
    coeffs = np.polyfit(group_data['ETc_Aquacrop'], group_data['Etc_Drone'], 1)
    x_vals = np.linspace(min(group_data['ETc_Aquacrop']), max(group_data['ETc_Aquacrop']), 100)
    y_vals = np.polyval(coeffs, x_vals)
    plt.plot(x_vals, y_vals, color=treatment_colors[treatment], linestyle='-', linewidth=1)
    
    # Calcular R y obtener la ecuación
    r = calculate_r(group_data['ETc_Aquacrop'], group_data['Etc_Drone'])
...
all_coeffs = np.polyfit(data['ETc_Aquacrop'], data['Etc_Drone'], 1)
all_x_vals = np.linspace(min(data['ETc_Aquacrop']), max(data['ETc_Aquacrop']), 100)
...
all_r = calculate_r(data['ETc_Aquacrop'], data['Etc_Drone'])
...
plt.xlabel('ET$_{c}$ water balance (mm d$^{-1}$)')
plt.ylabel('ET$_{c}$ energy balance (mm d$^{-1}$)')
...
plt.savefig('ETc_Simulated.png', dpi=600, bbox_inches='tight', pad_inches=0.1)
